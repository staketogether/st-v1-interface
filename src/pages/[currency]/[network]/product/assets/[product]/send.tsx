...
export default function Product({ product, assetData, chainId }: ProductProps) {
  return (
    <LayoutTemplate>
      <Metatags />
      <AssetsControl asset={product} assetData={assetData} chainId={chainId} type='send' />
    </LayoutTemplate>
  )
}
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
  ...
  const assetData = await fetchProductAssetData(`assets/${productSelected.chains[0]}/${productSelected.contractAddress}`)
